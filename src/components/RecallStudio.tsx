...
export default function RecallStudio({ items, initialText, mode = 'practice', onClose }: RecallStudioProps) {
    ...
    const navigateItems = (dir: 'prev' | 'next') => {
        ...
        setValidation(null);
        setIsRevealed(false);
        if (mode === 'review') setShowGuide(false);
                ...
                >
                    ...
                    <div className="pt-12 pb-4 px-4 md:px-8 flex flex-col items-center gap-4 md:gap-6 flex-1">
                            ...
                            <div className="flex-1 max-w-[500px] aspect-square relative group/canvas">
                                <AnimatePresence mode="wait">
                                    ...
                                    >
                                        ...
                                        {/* AI Analysis Result Overlay (Top-Left) */}
                                        <AnimatePresence>
                                            {validation && (
                                                <motion.div
                                                    ...
                                                    className={`absolute top-6 left-6 px-3 py-1.5 rounded-xl border backdrop-blur-md font-black shadow-lg text-[10px] z-50 ${validation.totalScore > 80 ? 'bg-green-500/20 border-green-500/30 text-green-400' : 'bg-red-500/20 border-red-500/30 text-red-400'}`}
                                                >
                                        ...
                                        {/* Minimalist Canvas Controls (Bottom-Right) - Monochrome Style */}
                                        <div className="absolute bottom-20 right-6 z-[60] flex flex-col gap-4 pointer-events-auto">
                                            <button
                                                onClick={undoPath}
                                                disabled={paths.length === 0}
                                                className="size-8 flex items-center justify-center transition-all hover:scale-110 active:scale-95 disabled:opacity-0 group"
                                            >
                                                <span className={`material-symbols-outlined text-[22px] transition-all duration-300 ${paths.length > 0 ? 'text-white/40 opacity-100' : 'text-white/10'}`}>
                                                    undo
                                                </span>
                                            </button>
                                            <button
                                                ...
                                                className="size-8 flex items-center justify-center transition-all hover:scale-110 active:scale-95 disabled:opacity-0 group"
                                            >
                                                <span className={`material-symbols-outlined text-[24px] transition-all duration-300 ${paths.length > 0 ? 'text-white/40 opacity-100' : 'text-white/10'}`}>
                                                    delete
                                                </span>
                                            </button>
                                        </div>

                                        {/* Swipe Zone (Bottom) - Hidden in Review Mode */}
                                        {mode !== 'review' && (
                                            <div
                        ...
                        <div className="w-full max-w-xl flex flex-col items-center gap-4 mt-auto">
                            ...
                            <div className="w-full grid grid-cols-4 gap-2 px-2">
                                {[
                                    ...
                                    <button
                                        ...
                                        disabled={paths.length === 0 || (mode === 'review' && !isRevealed)}
                            ...
                            <div className="w-full bg-white/5 border border-white/10 rounded-[32px] md:rounded-[40px] p-4 md:p-5 shadow-xl">
                                    ...
                                    <div className="flex items-center gap-2">
                                        {mode === 'review' ? (
                                            <button
                                                ...
                                                disabled={isRevealed || paths.length === 0}
                                                className={`px-4 h-11 md:h-12 rounded-2xl flex items-center gap-2 transition-all border ${isRevealed ? 'bg-green-500/10 border-green-500/20 text-green-400 opacity-50' : paths.length > 0 ? 'bg-primary border-primary text-white shadow-lg animate-pulse' : 'bg-white/5 border-white/5 text-gray-500'}`}
                                            >
                                                <span className="material-symbols-outlined text-xl">fact_check</span>
                                                <span className="text-[10px] font-black uppercase tracking-wider">Check Answer</span>
                                            </button>
                                        ) : (
                                            <button
                                                onClick={() => setShowGuide(!showGuide)}
                                                className={`px-4 h-11 md:h-12 rounded-2xl flex items-center gap-2 transition-all border ${showGuide ? 'bg-indigo-500/10 border-indigo-500/20 text-indigo-300 shadow-inner' : 'bg-white/5 border-white/5 text-gray-500'}`}
                                            >
